 ...
 * comment and replace it with the proper code here in the header file.  All
 * other functions should be defined in the .cpp file.
 * Comments starting with "***" are instructors to you - remove them from your
...
class Record {
  public:
    // Create a Record object, giving it a unique ID number by first
    // incrementing a static member variable then using its value as the
    // ID number. The rating is set to 0.
    ...
    static void save_ID_counter()
        {/*** fill this in */}
    // restore the ID counter from the value in the other static member
    // variable
  ...
  private:
    ...
    /* *** another static member variable for the backup value of iD_counter;
     * name is your choice */
...
// Output order is ID number followed by a ':' then medium, rating, title,
